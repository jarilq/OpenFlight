...
int main(void)
{
	...
	const int n_doubles = sizeof(datalog_double)/sizeof(double);
	const int n_floats = sizeof(datalog_float)/sizeof(float);
	const int n_longs = sizeof(datalog_long)/sizeof(long);
	...
	for (int i=0; i < n_fields; i++)
	{
		...
		if(err < 0)
		{
			printf("ERROR: Failed to create compound type\n");
			return -1;
		}
	}
	...
	{
		printf("ERROR: Unable to create packet table\n");
		H5Fclose(fid);
	}
	...
	for( int p=0; p<file_size/sizeof(datalog_t); p++)
	{
		...
		if(err < 0)
		{
			printf("ERROR: Data append failed");
			H5PTclose(ptable);
			H5Fclose(fid);
			return -1;
		}
	...
	if(err < 0)
	{
		printf("ERROR: Cannot close table");
		H5Fclose(fid);
		return -1;
	}
